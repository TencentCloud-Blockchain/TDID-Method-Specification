...
```json
{
  ...
  "verificationMethod": [
    {
      ...
      "type": "Secp256k1",
      "controller": "did:tdid:cb534-15:0x1e7115081c178459be8d3844dd129b2234edc6be",
  ...
  "authentication": [
       "did:tdid:cb534-15:0x1e7115081c178459be8d3844dd129b2234edc6be#keys-0"
  ],
  "service": []
}
```
...
1. The CURD operation of TDID on DID is exposed through Tencent Cloud public cloud API. Users need to use AK/SK authentication to call the operation method of DID. AK/SK authentication helps to further improve the security of TDID service.
2. TDID uses the BCOS blockchain network as a verifiable data registry. BCOS is a consortium chain with node access permission control. It is necessary to have a cloud account of a BCOS network node to call the operation method of DID.
3. The DID identifier of the TDID service is generated by the operation of the public key, so it supports the user's own private key and private key escrow modes to register DID:
...
4. The DID identifier of TDID is calculated from the public key through the address conversion algorithm, similar to the account address of Ethereum.
...
1. The DID document of TDID does not contain the user's private data, but only saves the user's public key data. The DID entity (DID Subject) cannot be identified from the DID document.
2. The user issues a verifiable credential through TDID to save his private information. The protection of user privacy depends on the security of VC.
